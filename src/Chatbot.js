...
🚀 **Instructions:**
- **The previous chat responses were all generated by YOU. Analyze them before generating the next response.**
- **Ensure consistency between past answers and the new response.**
...
- **If the query is about events, reference the user’s registered events.**
- **Ensure responses are natural, without repeating unnecessary greetings.**
- **Never respond with "I need your TechZite ID" if it has already been provided.**

💡 **Now, analyze the previous responses and generate a logical, context-aware reply!**
`;
    ...
    try {
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1/models/gemini-15-flash:generateContent?key=${GEMINI_API_KEY}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
